...
# (prefill_gpus, decode_gpus) -> point id (unique per point)
...
def linkcell(pid):
    ...
    if cl: out += ' [bench]({0}/client-logs/{1};tab=live_object)'.format(GCSF, cl)
    out += ' [cfg]({0}/{1}) [cfg-gcs]({2}/configs/{1};tab=live_object)'.format(GH, cfg, GCSF)
    return out
...
def summary_table(theirs, ours):
    ...
    for r in sorted(theirs, key=lambda x: (x['pg'], x['dg'], x['conc'])):
        ...
        pt = DSV4_PT.get((r['pg'], r['dg']), '')
        doc.append(f"| {pt} | {REGION.get((r['pg'], r['dg']), '')} | {topo} | {r['conc']} | InferenceMax | NATS | MNNVL (mooncake) | {r['total_per_gpu']:,.1f} | "
