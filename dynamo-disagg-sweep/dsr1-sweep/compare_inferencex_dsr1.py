...
# DSR1-FP4 point ids (p1-p8), ordered by (region, conc) — used in tables + Pareto figures
...
GCS = 'https://pantheon.corp.google.com/storage/browser/gke-aishared-gsc-dev/mlperf_results/nvfp4_20260519_run5/logs/logs'
GCSF = 'https://pantheon.corp.google.com/storage/browser/_details/gke-aishared-gsc-dev/mlperf_results/nvfp4_20260519_run5/logs/logs'
GH = 'https://github.com/alisachen-666/gcp-dynamo-cuj/blob/main/dynamo-disagg-sweep'
...
def linkcell(tag, pg, dg, conc):
    ...
    if cl:
        out += f' [bench]({GCSF}/client-logs/{cl};tab=live_object)'
    out += f' [cfg]({GH}/{cfg}) [cfg-gcs]({GCSF}/configs/{cfg};tab=live_object)'
    return out
...
def summary_table(theirs, ours, overlay=None):
    ...
    doc.append('| Pt | Pareto region | Prefill-Decode topology | Conc | Sources | Req plane | KV transfer | Total Tput (tok/s/GPU) | Input Tput (tok/s/GPU) | Output Tput (tok/s/GPU) | Median E2EL (s) | Median ITL (s) |')
    doc.append('|---|---|---|---|---|---|---|---|---|---|---|---|')
    ...
    for r in sorted(theirs, key=lambda x: (x['pg'], x['dg'], x['conc'])):
        ...
        doc.append(f"| {DSR1_PT.get((r['pg'], r['dg'], r['conc']), '')} | {REGION.get((r['pg'], r['dg']), '')} | {topo} | {r['conc']} | InferenceMax | NATS | MNNVL (NIXL) | {r['total_per_gpu']:,.1f} | "
...
**Setup**: cluster `cmcs-a4xmax` (cmcs-a4x-max), `a4x-maxgpu-4g-metal` (4x GB300/node);
