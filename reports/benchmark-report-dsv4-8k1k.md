# DeepSeek-V4-Pro FP4 GB300 8k1k Benchmark Report — GKE A4X Max
...
|---|---|---|---|---|---|---|---|---|---|
| p1 | 1 prefill TP4 (1 node) + 1 decode TP4 (1 node) | 8 (4/4) | 1 | 216.0 | 384 | 48.3 | 4.47 ms | 0.4 s | 4.8 s |
| p2 | ? | 28 (4/24) | 32 | 1044.8 | 6496 | 136.3 | 6.85 ms | 2.0 s | 8.5 s |
| p2 | ? | 28 (4/24) | 64 | 1727.3 | 10750 | 223.5 | 7.96 ms | 2.9 s | 10.6 s |
| p2 | ? | 28 (4/24) | 8 | 388.7 | 2415 | 51.0 | 5.23 ms | 0.7 s | 5.8 s |
| p3 | 1 prefill DEP4 (1 node) + 1 decode DEP8 (2 nodes) | 12 (4/8) | 256 | 5265.2 | 14040 | 877.7 | 10.75 ms | 22.5 s | 31.3 s |
| p4 | 1 prefill DEP4 (1 node) + 1 decode DEP16 (4 nodes) | 20 (4/16) | 256 | 3055.8 | 13581 | 424.5 | 8.28 ms | 24.0 s | 31.2 s |
| p5 | 2 prefill DEP4 (2 nodes) + 1 decode DEP8 (2 nodes) | 16 (8/8) | 512 | 7426.3 | 13202 | 1650.2 | 13.22 ms | 20.0 s | 32.0 s |
| p6 | 4 prefill DEP4 (4 nodes) + 1 decode DEP8 (2 nodes) | 24 (16/8) | 1024 | 9790.0 | 13053 | 3263.6 | 16.63 ms | 17.2 s | 32.7 s |
| p7 | 6 prefill DEP4 (6 nodes) + 1 decode DEP8 (2 nodes) | 32 (24/8) | 4096 | 11593.2 | 13741 | 5151.2 | 23.82 ms | 62.5 s | 84.1 s |

## Detailed results

### p1 — `disagg-low-latency-1p1d-tp4-tp4-mtp`

1 prefill TP4 (1 node) + 1 decode TP4 (1 node); 8 GPUs (4 prefill / 4 decode); concurrency 1; 10/10 completed in 48s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **216.0 tok/s/gpu** |
| Input token throughput per prefill GPU | **383.8 tok/s/gpu** |
| Output token throughput per decode GPU | **48.3 tok/s/gpu** |
| (aggregate total / input / output) | 1728 / 1535 / 193 tok/s |
| Request throughput | 0.21 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 633 | 433 | - | - | 2229 | 591 |
| TPOT (ms) | 4.47 | 4.04 | - | - | 7.56 | 1.14 |
| ITL (ms) | 15.7 | 15.7 | - | - | 16.9 | 0.7 |
| E2E latency (s) | 4.82 | 4.33 | - | - | 8.16 | 1.31 |

### p2 — `?`

?; 28 GPUs (4 prefill / 24 decode); concurrency 32; 320/320 completed in 91s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **1044.8 tok/s/gpu** |
| Input token throughput per prefill GPU | **6495.5 tok/s/gpu** |
| Output token throughput per decode GPU | **136.3 tok/s/gpu** |
| (aggregate total / input / output) | 29253 / 25982 / 3271 tok/s |
| Request throughput | 3.53 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 2167 | 1964 | - | - | 5918 | 952 |
| TPOT (ms) | 6.85 | 6.22 | - | - | 11.57 | 1.82 |
| ITL (ms) | 20.7 | 20.8 | - | - | 25.5 | 2.2 |
| E2E latency (s) | 8.50 | 8.32 | - | - | 13.31 | 1.90 |

### p2 — `?`

?; 28 GPUs (4 prefill / 24 decode); concurrency 64; 640/640 completed in 110s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **1727.3 tok/s/gpu** |
| Input token throughput per prefill GPU | **10750.1 tok/s/gpu** |
| Output token throughput per decode GPU | **223.5 tok/s/gpu** |
| (aggregate total / input / output) | 48365 / 43000 / 5365 tok/s |
| Request throughput | 5.82 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 3237 | 2943 | - | - | 8116 | 1506 |
| TPOT (ms) | 7.96 | 7.21 | - | - | 13.71 | 2.19 |
| ITL (ms) | 25.0 | 24.1 | - | - | 30.1 | 18.1 |
| E2E latency (s) | 10.56 | 10.05 | - | - | 17.84 | 2.56 |

### p2 — `?`

?; 28 GPUs (4 prefill / 24 decode); concurrency 8; 80/80 completed in 61s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **388.7 tok/s/gpu** |
| Input token throughput per prefill GPU | **2414.8 tok/s/gpu** |
| Output token throughput per decode GPU | **51.0 tok/s/gpu** |
| (aggregate total / input / output) | 10882 / 9659 / 1223 tok/s |
| Request throughput | 1.32 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 964 | 703 | - | - | 2925 | 525 |
| TPOT (ms) | 5.23 | 4.71 | - | - | 8.43 | 1.17 |
| ITL (ms) | 17.3 | 17.2 | - | - | 20.5 | 6.4 |
| E2E latency (s) | 5.82 | 5.37 | - | - | 10.12 | 1.41 |

### p3 — `disagg-mid-curve-1p1d-dep4-dep8-mtp`

1 prefill DEP4 (1 node) + 1 decode DEP8 (2 nodes); 12 GPUs (4 prefill / 8 decode); concurrency 256; 2560/2560 completed in 336s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **5265.2 tok/s/gpu** |
| Input token throughput per prefill GPU | **14040.3 tok/s/gpu** |
| Output token throughput per decode GPU | **877.7 tok/s/gpu** |
| (aggregate total / input / output) | 63183 / 56161 / 7022 tok/s |
| Request throughput | 7.62 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 21410 | 22534 | - | - | 34146 | 7488 |
| TPOT (ms) | 10.75 | 9.47 | - | - | 18.07 | 2.67 |
| ITL (ms) | 1983.5 | 1075.8 | - | - | 8778.5 | 2140.5 |
| E2E latency (s) | 31.31 | 32.31 | - | - | 46.98 | 7.87 |

### p4 — `disagg-mid-curve-1p1d-dep4-dep16-mtp`

1 prefill DEP4 (1 node) + 1 decode DEP16 (4 nodes); 20 GPUs (4 prefill / 16 decode); concurrency 256; 2560/2560 completed in 348s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **3055.8 tok/s/gpu** |
| Input token throughput per prefill GPU | **13581.1 tok/s/gpu** |
| Output token throughput per decode GPU | **424.5 tok/s/gpu** |
| (aggregate total / input / output) | 61116 / 54324 / 6792 tok/s |
| Request throughput | 7.37 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 23604 | 23997 | - | - | 36336 | 6587 |
| TPOT (ms) | 8.28 | 7.16 | - | - | 13.91 | 2.06 |
| ITL (ms) | 1481.2 | 792.0 | - | - | 6753.3 | 1648.7 |
| E2E latency (s) | 31.23 | 31.59 | - | - | 44.95 | 6.84 |

### p5 — `disagg-mid-curve-2p1d-dep4-dep8-mtp`

2 prefill DEP4 (2 nodes) + 1 decode DEP8 (2 nodes); 16 GPUs (8 prefill / 8 decode); concurrency 512; 5120/5120 completed in 358s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **7426.3 tok/s/gpu** |
| Input token throughput per prefill GPU | **13202.4 tok/s/gpu** |
| Output token throughput per decode GPU | **1650.2 tok/s/gpu** |
| (aggregate total / input / output) | 118820 / 105619 / 13201 tok/s |
| Request throughput | 14.32 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 19786 | 20008 | - | - | 44122 | 8315 |
| TPOT (ms) | 13.22 | 11.68 | - | - | 22.21 | 3.37 |
| ITL (ms) | 2486.0 | 1359.9 | - | - | 10883.4 | 2649.1 |
| E2E latency (s) | 31.97 | 32.13 | - | - | 53.96 | 8.83 |

### p6 — `disagg-mid-curve-4p1d-dep4-dep8-mtp`

4 prefill DEP4 (4 nodes) + 1 decode DEP8 (2 nodes); 24 GPUs (16 prefill / 8 decode); concurrency 1024; 10240/10240 completed in 362s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **9790.0 tok/s/gpu** |
| Input token throughput per prefill GPU | **13053.2 tok/s/gpu** |
| Output token throughput per decode GPU | **3263.6 tok/s/gpu** |
| (aggregate total / input / output) | 234959 / 208851 / 26109 tok/s |
| Request throughput | 28.33 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 17407 | 17244 | - | - | 35275 | 7908 |
| TPOT (ms) | 16.63 | 14.68 | - | - | 27.92 | 4.16 |
| ITL (ms) | 3117.1 | 1690.9 | - | - | 13761.9 | 3356.5 |
| E2E latency (s) | 32.71 | 32.42 | - | - | 53.54 | 8.76 |

### p7 — `disagg-high-conc-6p1d-dep4-dep8-mtp`

6 prefill DEP4 (6 nodes) + 1 decode DEP8 (2 nodes); 32 GPUs (24 prefill / 8 decode); concurrency 4096; 40958/40960 completed in 916s

| Throughput | Value |
|---|---|
| Total token throughput per GPU | **11593.2 tok/s/gpu** |
| Input token throughput per prefill GPU | **13740.6 tok/s/gpu** |
| Output token throughput per decode GPU | **5151.2 tok/s/gpu** |
| (aggregate total / input / output) | 370983 / 329774 / 41209 tok/s |
| Request throughput | 44.72 req/s |

| Latency | mean | median (p50) | p90 | p95 | p99 | std |
|---|---|---|---|---|---|---|
| TTFT (ms) | 62216 | 62506 | - | - | 96569 | 16488 |
| TPOT (ms) | 23.82 | 22.32 | - | - | 34.13 | 3.75 |
| ITL (ms) | 3466.7 | 1653.3 | - | - | 20864.1 | 4237.3 |
| E2E latency (s) | 84.14 | 84.29 | - | - | 119.02 | 17.03 |
...
## Comparison vs InferenceX published results
